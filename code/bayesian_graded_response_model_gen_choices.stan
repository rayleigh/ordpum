...
data {
  ...
  array[total_responses] int q_num_resp_v;
}
...
parameters {
  vector[num_ind] beta;
  vector<lower = 0>[num_questions] alpha;
  array[num_questions] ordered[K - 1] delta;
}
...
model {
  ...
  for (i in 1:total_responses) {
   ...
   if (response_v[i] < q_num_resp_v[i]) {
    ...
    } else if (response_v[i] == q_num_resp_v[i]) {
